...
app.get("/list", async (req: Request, res: Response) => {
  try {
    const text = "SELECT z.id, x.name, y.boardId FROM list";
...
app.get("/card", async (req: Request, res: Response) => {
  try {
    const text = 'SELECT c.id,c.titule c.descripcion,c.fecha_Date "isOwnerId" FROM card c JOIN card_users cu ON cu.listId = c.id WHERE cu.isOwnerId IS true';
    const result = await pool.query(text);
    res.status(200).json(result.rows);
  } catch (errors) {
    return res.status(400).json(errors);
...
app.post("/card", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const cardText = "INSERT INTO boards(titule,descripcion,fecha_Date,listId) VALUES($1,$2,$3,$4) RETURNING *";
    const cardValues = [cardDto.titule,cardDto.descripcion,cardDto.fecha_Date];
    const cardResult = await client.query(cardText, cardValues);

    const card_UserText =
      "INSERT INTO card_users(cardId, userId, isOwner) VALUES($1, $2, $3)";
    const cardUserValues = [
      cardResult.rows[0].id,
      cardDto.isOwnerId,
      true,
    ];
    await client.query(card_UserText, cardUserValues);
